...
import { createSetupIntent, listPaymentMethods,deleteCard,detachePaymentMethod } from './customers';
import {
  createSubscription,
  cancelSubscription,
  listSubscriptions,
...
// Create a charge and new Subscription
app.post(
  '/subscriptions/',
  runAsync(async (req: Request, res: Response) => {
    const user = validateUser(req);
    ...
    if(subscription==="404")
    {res.send('Customer already subscribed to a plan!! Please cancel existing one').status(500);}
