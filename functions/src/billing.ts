...
export async function createSubscription(
  ...
  const checkSubscription = await listSubscriptions(userId);
  if (checkSubscription.data.length>0)
  {
    return "404";
...
export async function cancelSubscription(
  ...
  //if(subscription.current_period_end===true)
...
export async function updateSubscription(
  ...
  if ((await subscription).cancel_at_period_end===true) {
    await db
      .collection(userId)
      .doc("Profile")
      .update({
        activePlans: firestore.FieldValue.arrayRemove((await subscription).id),
      });
  }
